...
func (r *Repository) GetByID(ctx context.Context, id uuid.UUID) (*domain.School, error) {
	...
	err := r.pool.QueryRow(ctx, `
		SELECT id, name, code, COALESCE(address,''), COALESCE(phone,''), COALESCE(email,''), working_days_per_month, COALESCE(dice_code,''), created_at, updated_at
		FROM schools WHERE id = $1`, id,
	).Scan(&s.ID, &s.Name, &s.Code, &s.Address, &s.Phone, &s.Email, &s.WorkingDaysPerMonth, &s.DiceCode, &s.CreatedAt, &s.UpdatedAt)
...
func (r *Repository) List(ctx context.Context, schoolID *uuid.UUID, limit, offset int) ([]domain.School, int, error) {
	...
	for rows.Next() {
		var s domain.School
		if err := rows.Scan(&s.ID, &s.Name, &s.Code, &s.Address, &s.Phone, &s.Email, &s.WorkingDaysPerMonth, &s.DiceCode, &s.CreatedAt, &s.UpdatedAt); err != nil {
			return nil, 0, err
		}
...
func (r *Repository) Update(ctx context.Context, s *domain.School) error {
	tag, err := r.pool.Exec(ctx, `
		UPDATE schools SET name=$2, code=$3, address=$4, phone=$5, email=$6, working_days_per_month=$7, dice_code=$8, updated_at=NOW()
		WHERE id=$1`,
		s.ID, s.Name, s.Code, s.Address, s.Phone, s.Email, s.WorkingDaysPerMonth, s.DiceCode,
	)
	if err != nil {
		return err
	}
